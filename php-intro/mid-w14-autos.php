...
$url = getUrl('http://www.php-intro.com/exam/mid-w14-autos');
// $url = getUrl('http://localhost/~csev/mid-w14-autos');
//$url = getUrl('');
$grade = 0;
...
line_out("-- this autograder expects the form field makes to be:");
line_out("-- make, make, year, and miles");
...
$model = rand(1,100);
$year = rand(1,100);
line_out("Entering make=$make, model=$model, year=$year");
...
$model = rand(1,100);
$year = rand(1,100);
line_out("Editing make=$make, model=$model, year=$year");
...
line_out("Cleaning up old Ford records...");
while (True ) {
